...
static int adf4355_powerdown(struct xtrxll_dev* dev)
{
	uint32_t adf4 = MAKE_ADF4355_R4(6, 0, 0, 1, 0, 2, 0, 1, 1, 1, 0, 0);
	return adf4355_spi(dev, adf4);
}

static int adf4355_tune(struct xtrxll_dev* dev, uint64_t outfreq, unsigned fref, unsigned flags)
{
	int res;
...
#define VCO_MAX 6600e6
	...
	//if (1) {
		...
		for (unsigned i = 0; i < 12; i++) {
			XTRXLLS_LOG("OCTO", XTRXLL_INFO_LMS, "ADF OUT %08x\n", adfregs[i]);
			int res = adf4355_spi(dev, adfregs[i]);
			if (res) {
				return res;
			}
		}
	...
	uint32_t adfregs2[] = {
		//MAKE_ADF4355_R10(adc_div, 1, 1),
		MAKE_ADF4355_R4(6, ref_doubler, 0, 1/*R*/, 0 /* db rfdiv*/, icp_idx, 0, 1, 1, 0, 0, 1),
		MAKE_ADF4355_R2(0, 130),
		MAKE_ADF4355_R1(frac),
	...
	for (unsigned i = 0; i < sizeof(adfregs2)/sizeof(adfregs2[0]); i++) {
		int res = adf4355_spi(dev, adfregs2[i]);
		if (res) {
			return res;
		}
	}
...
static int lms7octo_lo_tune(struct xtrx_lms7octo *dev)
{
	...
	int res = adf4355_tune(dev->master, dev->mdev->lo, fref,
...
static int lms7octo_lo_pd(struct xtrx_lms7octo *dev)
{
	...
	dev->mdev->flags[0] &= ~ADF_INIT;
	return adf4355_powerdown(dev->master);
}
